    ...
    const fetchAcademicSessions = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5001/api/grading/sessions",
          {
            headers: {
  ...
  useEffect(() => {
    ...
    const fetchClassPerformance = async () => {
      try {
        setLoading(true);
        const response = await axios.get(
          `http://localhost:5001/api/analytics/class-summary/${selectedClass}?academic_session_id=${selectedSession}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  useEffect(() => {
    ...
    const fetchSubjectPerformance = async () => {
      try {
        setLoading(true);
        const response = await axios.get(
          `http://localhost:5001/api/examgrading/subject-performance/${selectedClass}?examination_id=${selectedExam}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  useEffect(() => {
    ...
    const fetchSubjectAnalysis = async () => {
      try {
        setLoading(true);
        const response = await axios.get(
          `http://localhost:5001/api/analytics/subject-analysis/${selectedSubject}?class_id=${selectedClass}&academic_session_id=${selectedSession}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },

    ...
    <div className="space-y-6">
      ...
      {student?.medical_conditions && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          ...
          <p className="mt-2 text-sm text-yellow-700">{student.medical_conditions}</p>
        </div>
      )}
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <p className="mt-1">{student?.blood_group || 'Not specified'}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-500">Allergies</label>
            <p className="mt-1">{student?.allergies.length === 0 ? 'None reported' : student?.allergies}</p>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <p className="mt-1 font-medium">{student?.guardian?.name}</p>
            <p className="text-sm text-gray-600">{student?.guardian?.phone}</p>

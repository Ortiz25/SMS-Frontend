...
const ExamTypesTab = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (examTypesResponse.data.success) {
          ...
          toast.error(
            examTypesResponse.data.message || "Failed to fetch exam types"
          );
        }
        ...
        if (gradingSystemsResponse.data.success) {
          ...
          toast.error(
            gradingSystemsResponse.data.message ||
              "Failed to fetch grading systems"
          );
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        response = await academicSettingsAPI.updateExamType(
          editingId,
          examTypeData
        );
  ...
  const handleEdit = (examType) => {
    ...
    setFormData({
      ...
      gradingSystemId: examType.grading_system_id
        ? examType.grading_system_id.toString()
        : "",
    });
  ...
  const filteredGradingSystems = gradingSystems.filter(
    (system) =>
      system.curriculum_type === formData.curriculumType && system.is_active
  );
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Configure examination categories and weighting
            </p>
          </div>
          <button
      ...
      <div className="p-4 sm:p-6">
                ...
                {examTypes.map((examType) => (
                  <tr key={examType.id}>
                    ...
                    <td className="py-3 px-4">
                      {examType.grading_system_name || "None"}
                    </td>
                    <td className="py-3 px-4">
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${
                          examType.is_national_exam
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg text-sm text-gray-600">
          ...
          <div className="space-y-1">
            <p>
              • Exam types determine how assessments are categorized and
              weighted
            </p>
            <p>
              • Each exam type must be linked to a grading system from the same
              curriculum
            </p>
            <p>
              • Weight percentages determine how much each exam contributes to
              final grades
            </p>
            <p>
              • National exams like KCPE and KCSE have special grading
              considerations
            </p>
          </div>
        </div>

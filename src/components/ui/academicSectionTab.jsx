...
const AcademicSessionsTab = () => {
  ...
  const fetchAcademicSessions = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(
          response.data.message || "Failed to fetch academic sessions"
        );
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await academicSettingsAPI.createAcademicSession(
        sessionData
      );
      if (response.data.success) {
        ...
        toast.error(
          response.data.message || "Failed to create academic session"
        );
      }
    } catch (error) {
  ...
  const handleEdit = (session) => {
    ...
    if (session.status === "completed") {
      toast.warning(
        "Completed sessions cannot be edited. Create a new session if needed."
      );
      return;
    }
  ...
  const toggleCurrentSession = async (id) => {
    ...
    if (session && session.status === "completed") {
      toast.warning(
        "Completed sessions cannot be set as current. Please activate the session first."
      );
      return;
    }
  ...
  const updateSessionStatus = async (id, newStatus) => {
    ...
    try {
      ...
      const response = await academicSettingsAPI.updateSessionStatus(
        id,
        newStatus
      );
      ...
      if (response.data.success) {
        ...
        const sessionToUpdate = academicSessions.find(
          (session) => session.id === id
        );
        if (newStatus === "completed" && sessionToUpdate.is_current) {
          toast.warning(
            "You've marked the current session as completed. Please set another active session as current."
          );
        }
  ...
  return (
    <div>
      ...
      <div className="p-4 sm:p-6">
          ...
          <div className="bg-blue-50 p-4 mb-6 rounded-lg border border-blue-100">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex flex-col-reverse sm:flex-row justify-end gap-2">
                <button
                  type="button"
                  className="mt-2 sm:mt-0 px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-md"
        ...
        {/* Academic Sessions Table - Desktop/Tablet View */}
        {!loading && (
          <>
              ...
              <table className="min-w-full bg-white">
                ...
                <tbody className="divide-y divide-gray-200">
                  {academicSessions.map((session) => (
                    <tr
                      key={session.id}
                      className={session.is_current ? "bg-blue-50" : ""}
                    >
                            ...
                            {session.status !== "scheduled" &&
                              session.status !== "completed" && (
                                <button
                                  ...
                                  onClick={() =>
                                    updateSessionStatus(session.id, "scheduled")
                                  }
                                  disabled={loading}
            ...
            <div className="sm:hidden space-y-4">
              {academicSessions.length === 0 && !loading ? (
                <div className="py-4 px-4 text-center text-gray-500 bg-white rounded-lg border border-gray-200">
                  No academic sessions found. Create one to get started.
                </div>
              ) : (
                academicSessions.map((session) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <button
                        className={`p-1 ${
                          session.status === "completed"
                            ? "opacity-50 cursor-not-allowed"
                            : "hover:bg-white rounded bg-opacity-50"
                        }`}
                        onClick={() => handleEdit(session)}
                        disabled={loading || session.status === "completed"}
                      >
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      ...
                      {!session.is_current && (
                        <button
                          className={`py-1 px-2 text-xs rounded-md flex items-center ${
                            session.status === "completed"
                              ? "bg-gray-100 text-gray-400 cursor-not-allowed"
                      ...
                      {session.status !== "active" &&
                        session.status !== "completed" && (
                          <button
                            className="py-1 px-2 text-xs bg-green-100 text-green-800 rounded-md flex items-center"
                            onClick={() =>
                              updateSessionStatus(session.id, "active")
                            }
                            disabled={loading}
                      ...
                      {session.status !== "scheduled" &&
                        session.status !== "completed" && (
                          <button
                            className="py-1 px-2 text-xs bg-yellow-100 text-yellow-800 rounded-md flex items-center"
                            onClick={() =>
                              updateSessionStatus(session.id, "scheduled")
                            }
                            disabled={loading}
                      ...
                      {session.status !== "completed" && (
                        <button
                          className="py-1 px-2 text-xs bg-gray-100 text-gray-800 rounded-md flex items-center"
                          onClick={() =>
                            updateSessionStatus(session.id, "completed")
                          }
                          disabled={loading}
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg text-sm text-gray-600">
          ...
          <div className="space-y-1">
            <p>
              ...
              <span className="font-medium text-green-700">Active</span> - The
              session is currently in progress or ready to be used
            </p>
            <p>
              <span className="font-medium text-yellow-700">•</span>{" "}
              <span className="font-medium text-yellow-700">Scheduled</span> -
              Future session that is planned but not yet active
            </p>
            <p>
              <span className="font-medium text-gray-700">•</span>{" "}
              <span className="font-medium text-gray-700">Completed</span> -
              Past session that has been concluded
            </p>
            <p>
              <span className="font-medium text-blue-700">•</span>{" "}
              <span className="font-medium text-blue-700">Current</span> - The
              session that is currently selected across the system
            </p>
          </div>
          <p className="mt-2">
            Only one session can be set as current at a time. This determines
            which academic period is used for new entries across the system.
          </p>
          <p className="mt-1">
            <strong>Note:</strong> Once a session is marked as "Completed", it
            becomes read-only and cannot be modified. This preserves the
            integrity of historical academic data.
          </p>
        </div>
      </div>
    </div>

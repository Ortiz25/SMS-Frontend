...
const AttendanceEntryModal = ({ isOpen, onClose }) => {
  ...
  const fetchCurrentSession = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:5001/api/sessions/academic-sessions/current",
        {
          headers: {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `http://localhost:5001/api/students/by-class?classId=${selectedClass}&stream=${selectedStream}`, 
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
  ...
  const checkExistingAttendance = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `http://localhost:5001/api/attendance/class/${selectedClass}?date=${selectedDate}`,
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5001/api/attendance/bulk",
        { attendanceRecords },
        {
          headers: {
            Authorization: `Bearer ${token}`,

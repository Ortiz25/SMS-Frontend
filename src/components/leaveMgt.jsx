...
const LeaveManagement = () => {
  ...
  const fetchLeaveStats = async () => {
    try {
      const statsResponse = await axios.get("http://localhost:5001/api/leaves/", {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      ...
      const balanceResponse =
        !isAdmin && currentTeacherId
          ? await axios.get(`http://localhost:5001/api/leaves/balances/${currentTeacherId}`, {
              headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
              },
  ...
  const handleApproveLeave = async (data) => {
    try {
      const response = await axios.patch(
        `http://localhost:5001/api/leaves/${data.id}/status`,
        {
          status: "approved",
        },
        {

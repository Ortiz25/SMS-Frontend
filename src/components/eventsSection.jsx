          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <div className="flex items-center text-gray-600 mb-3">
                ...
                <span>
                  {format(
                    parseISO(viewingEvent.event_date),
                    "EEEE, MMMM d, yyyy"
                  )}
                </span>
              </div>
                ...
                <div className="flex items-center text-gray-600 mb-3">
                  ...
                  <span>
                    {viewingEvent.start_time &&
                      format(
                        parseISO(`2000-01-01T${viewingEvent.start_time}`),
                        "h:mm a"
                      )}
                    {viewingEvent.start_time && viewingEvent.end_time && " - "}
                    {viewingEvent.end_time &&
                      format(
                        parseISO(`2000-01-01T${viewingEvent.end_time}`),
                        "h:mm a"
                      )}
                  </span>
                </div>
            ...
            <div>
              ...
              <div className="text-sm text-gray-500 mt-4">
                ...
                Created at:{" "}
                {format(
                  parseISO(viewingEvent.created_at),
                  "MMM d, yyyy h:mm a"
                )}
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-4">
              ...
              <div className="flex items-center">
                ...
                >
                  Make this event visible to all users (including parents and
                  students)
                </label>
              </div>
        ...
        <div className="space-y-4">
          {filteredEvents.length === 0 ? (
            ...
            filteredEvents.map((event) => (
              <div
                key={event.id}
                className="bg-white p-4 rounded-md border border-gray-200 shadow-sm hover:shadow transition-shadow duration-200"
              >
                    ...
                    <div className="flex flex-wrap items-center text-sm text-gray-500 gap-3 mb-2">
                      ...
                      {event.start_time && (
                        <span className="flex items-center">
                          ...
                          {event.end_time &&
                            ` - ${format(
                              parseISO(`2000-01-01T${event.end_time}`),
                              "h:mm a"
                            )}`}
                        </span>
                      )}

...
const WeeklySchedule = ({ timetableData, selectedClass, selectedTeacher, selectedRoom }) => {
  ...
  const handleEditSave = async (editedData) => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/timetable/${editedData.id}`, {
        method: "PUT",
        headers: {
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/timetable/${selectedSchedule.id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },

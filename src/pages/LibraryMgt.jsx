...
const LibraryManagement = () => {
  ...
  const handleBorrow = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5001/api/library/books/${selectedBook.id}/borrow`,
        {
          method: "POST",
          headers: {
  ...
  const handleDeleteBook = async (bookId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5001/api/library/books/${bookId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleSaveBook = async () => {
    try {
      const url = isEditing
        ? `http://localhost:5001/api/library/books/${bookFormData.id}`
  ...
  const handleReturn = async (bookId) => {
    try {
      const response = await fetch(
        `http://localhost:5001/api/library/books/${bookId}/return`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
export async function loader({ params, request }) {
  ...
  try {
    ...
    const apiUrl = `http://localhost:5001/api/library/books`;

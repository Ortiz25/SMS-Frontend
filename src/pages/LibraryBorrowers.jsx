...
const Borrowers = () => {
    ...
    try {
      ...
      const endpoint =
        filter === "overdue"
          ...
          : searchQuery
          ? `/backend/api/library/borrowers/search?search=${encodeURIComponent(
              searchQuery
            )}`
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to fetch borrowers: ${response.status} ${errorText}`
        );
      }
  ...
  const handleReturn = async (bookId) => {
    try {
      ...
      setStats((prev) => ({
        ...
        overdue:
          prev.overdue -
          (borrowers.find((b) => b.book_id === bookId)?.status === "overdue"
            ? 1
            : 0),
      }));
  ...
  const handleExtendLoan = async (book) => {
    try {
      ...
      alert(
        `Loan period extended for "${
          book.title
        }" until ${newDueDate.toLocaleDateString()}`
      );
    } catch (err) {
  ...
  const currentItems = filteredBorrowers.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  const overdueCount = borrowers.filter(
    (book) => new Date(book.due_date) < new Date()
  ).length;
      ...
      <div className="p-6 space-y-6 bg-white rounded-lg shadow-sm m-4">
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-4">
            <Link
              to="/library"
              className="p-2 rounded-full bg-gray-100 hover:bg-gray-300 transform hover:scale-105 hover:-translate-y-0.5 transition duration-200 ease-in-out"
            >
          ...
          <div className="flex gap-4">
            ...
            <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
              ...
              {
                borrowers.filter((b) => new Date(b.due_date) >= new Date())
                  .length
              }{" "}
              Active
            </span>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 bg-gray-50 rounded-lg">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Currently no books are checked out from the library.
            </p>
            <div className="mt-4">
              <Link
                to="/library"
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          {book.fine_amount > 0 && (
                            <div className="mt-1 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                              ...
                              Fine: KES{" "}
                              {parseFloat(book.fine_amount).toFixed(2)}
                            </div>
                          )}
                        </td>
